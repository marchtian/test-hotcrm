...
export const pipeline: Record<string, ObjectTranslationData> = {
  crm_lead: {
    ...
    fields: {
      ...
      demand_type: { label: '需求类型', options: { new_system: '新建系统', upgrade: '升级改造', consulting: '咨询', operations: '运维服务', integration: '系统集成', other: '其他' } },
      expected_amount: { label: '预计金额' },
      approval_status: {
        label: '审批状态',
  ...
  crm_opportunity: {
    ...
    fields: {
      ...
      subcontract_plan: { label: '分包信息', options: { none: '无分包', partial: '部分分包', full: '整体分包' } },
      payment_terms_notes: { label: '付款条款' },
      subcontract_notes: { label: '下包说明' },
